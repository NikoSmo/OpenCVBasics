...
def store_raw_images(image_folder, image_url_list, image_starting_index = 1):
    ...
    for image in image_list.split('\n'):
        try:
            print(image)
            print (image_index)
            urllib.request.urlretrieve(image, image_folder + '/' + str(image_index) + '.jpg')
            img  = cv2.imread(image_folder + '/' + str(image_index) + '.jpg', cv2.IMREAD_GRAYSCALE)
            # should be larger than samples / pos pic
            resized_img = cv2.resize(img, (100,100))
            cv2.imwrite(image_folder + '/' + str(image_index) + '.jpg', resized_img)
        ...
        for img in os.listdir(file_type):
            for false_image in os.listdir(false_image_folder):
                try:
                    ...
                    false_image = cv2.imread(false_image_folder + '/' + str(false_image))
